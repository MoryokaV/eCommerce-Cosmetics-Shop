...
class _OrderSummaryState extends State<OrderSummary>
  ...
  double calcPrice(List<Product> cartProducts) {
    double total = 0;

    for (int i = 0; i < widget.cart.length; i++)
      total += cartProducts[i].price * widget.cart[i].productQuantity;
  ...
  String getOrderDescription(List<Product> cartProducts) {
    String desc = "";
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          ...
          for (int i = 0; i < snapshot.data!.docs.length; i++) {
            cartProducts.add(Product.fromSnapshot(snapshot.data!.docs[i]));
          }
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                  color: Colors.grey[500],
                                  fontSize:
                                      Responsive.safeBlockHorizontal * 4.5,
                                  fontFamily: "Arial",
                        ...
                        Spacer(),
                        Text(
                          calcPrice(cartProducts).toString() + " RON",
                          style: TextStyle(
                            color: kAccentColor,
                  ...
                  if (summaryList) buildSummaryListDetails(cartProducts),
                  GestureDetector(
                    onTap: () {
                      setState(() {
                        widget.height = 0.06;
                          ...
                          Navigator.push(
                            ...
                            CupertinoPageRoute(
                              builder: (_) => OrderScreen(
                                order: Order(
                                  number: widget.orders != 0
                                      ? widget.orders + 1
                                      : 1,
                                  value: calcPrice(cartProducts),
                                  description:
                                      getOrderDescription(cartProducts),
                                  date: dateTime,
                                ),
                              ),
                            ),
  ...
  Widget buildSummaryListDetails(List<Product> cartProducts) {
    return Container(
